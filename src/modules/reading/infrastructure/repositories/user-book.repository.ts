...
export class UserBookRepository implements IUserBookRepository {
   ...
   */
  public async findAllWithBook(query: QueryBase): Promise<{
    ...
    // find all user books with book
    let userBookWithBooks: Array<UserBookModel & { book: BookModel }> = [];
    try {
      userBookWithBooks = await this.postgresqlService.userBook.findMany({
        where: whereClause,
        orderBy: orderClause,
        take: take,
        skip: skip,
    ...
    return {
      data: userBookWithBooks.map((userBookWithBook) => ({
        userBook: UserBookMapper.toEntity(userBookWithBook),
        book: BookMapper.toEntity(userBookWithBook.book),
      })),
