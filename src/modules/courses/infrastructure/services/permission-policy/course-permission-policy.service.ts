...
/**
 * Course permission policy service
 * @description Course permission policy service which is used to check permissions for course resources.
 */
@Injectable()
export class CoursePermissionPolicyService
  implements IPermissionPolicy<CourseEntity>
{
  /**
  ...
  private generalRule(
    ...
    // A user can access a course if they are admin or the creator of the course
    return (
      user.getRole().isAdmin() ||
      user.getId().getId() === resource.getId().getId()
    );
  }
  ...
  public canModify(
    ...
    // always allow modification to the course
  ...
  public canModifyCollection(user: UserEntity): boolean {
    // always allow modification to the collection
    return true;
  }
}
